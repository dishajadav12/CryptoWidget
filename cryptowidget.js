...
fetch(apiUrl)
  ...
  .then((data) => {
    ...
    const current_price = data.market_data.current_price[currencyType.toLowerCase()];
    const total_volume = data.market_data.total_volume[currencyType.toLowerCase()];
    ...
    const widgetHTML = `
      <div class="coingecko-token-widget">
        ...
        <div class="section-2">
          ${showRank ? `<div class="sub-section-1">RANK <br /><span class='sub-sec-value'>${market_cap_rank}</span></div>` : ''}
          ${showMarketCap ? `<div class="sub-section">MARKET CAP <br /><span class='sub-sec-value'>$${market_cap.toLocaleString()}</span><span class='currency-type'>${currencyType}</span></div>` : ''}
          ${showVolume ? `<div class="sub-section-2">VOLUME <br /><span class='sub-sec-value'>$${total_volume.toLocaleString()}</span><span class='currency-type'>${currencyType}</span></div>` : ''}
        </div>
        <div class="section-3">
